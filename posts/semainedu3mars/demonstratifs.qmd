...
<body>
    ...
    <script type="text/babel">
        ...
        const refreshToken = async () => {
            ...
            if (!storedRefreshToken) window.location.href = "/login.html";
        ...
        // Function to show no access message based on language
        const showNoAccessMessage = (language) => {
            const messages = {
                en: {
                    title: "You don't have access to this page.",
                    message: "Please <a href='https://frenchwithkunal.ca/payment.html'>subscribe</a> to gain access.",
                },
                fr: {
                    title: "Vous n'avez pas accès à cette page.",
                    message: "Veuillez <a href='https://frenchwithkunal.ca/payment.html'>vous abonner</a> pour obtenir l'accès.",
                },
            };

            const { title, message } = messages[language] || messages.en; // Default to English if language is not recognized
        ...
        const ProfilePage = () => {
            ...
            useEffect(() => {
                // Detect language from URL or query parameter
                const urlParams = new URLSearchParams(window.location.search);
                const lang = urlParams.get("lang") || "en"; // Default to English if no language is specified
                setLanguage(lang);

                const storedToken = localStorage.getItem('accessToken');
                if (!storedToken) showNoAccessMessage(lang);

                const fetchData = async () => {
                    const data = await fetchWithAuth("https://frenchwithkunal-cdf6ea3f88ce.herokuapp.com/api/v1/auth/me", { method: "POST" });
                    if (!data.data.hasAccess) showNoAccessMessage(lang);
                };

                if (storedToken) fetchData();
            }, []);

            return <div></div>;
        };

        ReactDOM.render(<ProfilePage />, document.getElementById('react-root'));
    </script>
